...
<img src="docs/images/dashboard.png" width="880" alt="工作台：合同状态统计与最近合同">
...
浅色主题、响应式布局，下面都是系统真实界面（示例数据，非真实合同内容）。

<table>
<tr>
<td width="50%"><img src="docs/images/login.png" alt="登录页"><br><sub><b>登录</b>：单页应用，首次登录强制改密</sub></td>
<td width="50%"><img src="docs/images/contracts.png" alt="我的合同"><br><sub><b>我的合同</b>：搜索、状态筛选、分页，状态用彩色徽标区分</sub></td>
</tr>
<tr>
<td width="50%"><img src="docs/images/contract-fill.png" alt="在线填报"><br><sub><b>在线填报</b>：按字段类型渲染控件，默认值自动预填；关闭弹窗会先自动保存草稿</sub></td>
<td width="50%"><img src="docs/images/template-detail.png" alt="模板版本管理"><br><sub><b>模板版本管理</b>：上传母版、字段配置、校验、试填、发布；已发布版本不可原地改</sub></td>
</tr>
<tr>
<td width="50%"><img src="docs/images/audit.png" alt="审计日志"><br><sub><b>审计日志</b>：创建/修改/发布/定稿/下载/作废/删除全程留痕</sub></td>
<td width="50%"></td>
</tr>
</table>
...
### 界面
- **浅色主题**：白色卡片 + 浅灰蓝底、蓝色强调色，状态徽标（填报中/已定稿/已签署/已作废）配色区分，表格行悬停、弹窗头部吸顶。
- **零构建单页应用**：原生 JS + CSS 由后端 `static/` 直接提供，改样式刷新即生效，不需要 npm/打包工具。
- **响应式**：≤1000/760/560px 三档自适应（统计卡两列→一列、表单单列、小屏收起导航）。
- **细致的交互状态**：禁用按钮用"禁止"光标（不再用会一直转圈的忙碌光标），提交中的按钮有忙碌光标与"正在…"文案；分页按钮禁用时提示"已经是第一页/最后一页"。
- **防误关**：填报页改动后，点弹窗外面 / 按 Esc / 点关闭会先自动保存为草稿再关；保存失败则保留页面并给出"放弃修改"的出口；刷新或关闭标签页前还有浏览器原生提醒。
...
```bash
mvn package
node scripts/test-api-guards.cjs  # 19 项检查，全部通过才输出 ALL_PASS
```
