...
public class ContractController {
    ...
    @GetMapping("/contracts") Object list(HttpServletRequest r,@RequestParam(defaultValue="") String q,@RequestParam(defaultValue="") String status,@RequestParam(defaultValue="1") int page,@RequestParam(defaultValue="20") int size) {
        // page/size 双向夹紧：负数或超大页码会让 (page-1)*size 溢出成负 OFFSET，数据库直接报错（500）。
        size=Math.max(1,Math.min(size,100)); page=Math.max(1,Math.min(page,1_000_000)); boolean all=admin(r); String owner=all?"%":str(auth.actor(r),"id"); String term="%"+q.toLowerCase(Locale.ROOT)+"%";
    ...
    @PostMapping("/contracts/{id}/generate") @ResponseStatus(HttpStatus.ACCEPTED) Object generate(@PathVariable String id,HttpServletRequest r) { return auth.write(r,()->{
        ...
        db.update("INSERT INTO contract_revisions(id,contract_id,revision_no,template_version_id,data_snapshot,draft_lock_version,template_hash,schema_hash,render_profile_id,created_by) VALUES (?,?,?,?,?,?,?,?,?,?)",rid,id,no,v.get("id"),db.stringify(values),num(c,"lock_version"),str(db.one("SELECT sha256 FROM files WHERE id=?",v.get("docx_file_id")),"sha256"),v.get("schema_hash"),generation.renderProfile(),auth.actor(r).get("id"));
    ...
    @GetMapping("/contracts/{id}/revisions/{rid}/preview") Object preview(@PathVariable String id,@PathVariable String rid,HttpServletRequest r) {
        var c=contract(id,r); var rev=revision(rid); require(id.equals(rev.get("contract_id")),404,"NOT_FOUND","修订不存在"); var j=db.find("SELECT * FROM generation_jobs WHERE revision_id=? AND state='SUCCEEDED'",rid);
        require(j!=null,409,"PREVIEW_NOT_READY","该修订还没有生成成功的全文（可能仍在排队、生成中或已失败），请先生成全文");
    ...
    @PostMapping("/contracts/{id}/revisions/{rid}/review") Object review(@PathVariable String id,@PathVariable String rid,@RequestBody Map<String,Object> b,HttpServletRequest r) { return auth.write(r,()->{
        contract(id,r); var rev=revision(rid); require(id.equals(rev.get("contract_id")),404,"NOT_FOUND","修订不存在"); require(Boolean.TRUE.equals(b.get("confirmed")),422,"CONFIRMATION_REQUIRED","请确认已查看完整 PDF"); var j=db.find("SELECT * FROM generation_jobs WHERE revision_id=? AND state='SUCCEEDED'",rid); require(j!=null,409,"PREVIEW_NOT_READY","该修订还没有生成成功的全文，请先生成全文再确认"); var pdf=db.find("SELECT * FROM files WHERE id=?",j==null?null:j.get("pdf_file_id")); require(pdf!=null,409,"PREVIEW_NOT_READY","生成产物已被清理，请重新生成全文"); require(db.count("SELECT COUNT(*) FROM preview_views WHERE user_id=? AND job_id=?",auth.actor(r).get("id"),j.get("id"))>0,422,"PREVIEW_REQUIRED","请先打开本次完整 PDF");
    ...
    @PostMapping("/contracts/{id}/finalize") Object finalize(@PathVariable String id,@RequestBody Map<String,Object> b,HttpServletRequest r) { return auth.write(r,()->{
        ...
        var j=db.find("SELECT * FROM generation_jobs WHERE revision_id=? AND state='SUCCEEDED'",rid); require(j!=null,409,"PREVIEW_NOT_READY","该修订没有生成成功的全文，无法定稿"); var rec=db.find("SELECT * FROM review_receipts WHERE id=? AND user_id=? AND revision_id=? AND expires_at>?",receipt,auth.actor(r).get("id"),rid,at(Instant.now())); require(rec!=null,409,"REVIEW_OUTDATED","确认凭据无效或已过期，请重新预览并确认"); var pdf=db.find("SELECT * FROM files WHERE id=?",j==null?null:j.get("pdf_file_id")); require(pdf!=null,409,"PREVIEW_NOT_READY","生成产物已被清理，请重新生成全文");
    ...
    @GetMapping("/contracts/{id}/download") Object download(@PathVariable String id,@RequestParam(defaultValue="pdf") String format,HttpServletRequest r) { var c=contract(id,r); require(Set.of("docx","pdf").contains(format),422,"INVALID_FORMAT","文件格式无效"); String rid=str(c,"finalized_revision_id"); require(!rid.isBlank(),409,"CONTRACT_NOT_FINALIZED","合同尚未定稿"); var j=db.find("SELECT * FROM generation_jobs WHERE revision_id=? AND state='SUCCEEDED'",rid); require(j!=null,409,"PREVIEW_NOT_READY","定稿修订的生成产物不存在，请联系管理员"); return templates.download(str(j,format.equals("pdf")?"pdf_file_id":"docx_file_id"),str(c,"contract_no")+"-FINAL-V"+revision(rid).get("revision_no")+"."+format,false,r,"CONTRACT_DOWNLOADED",id); }
    /** 历史入口只放行被定稿冻结的那个修订，未定稿的中间草稿不可下载。 */
    @GetMapping("/contracts/{id}/revisions/{rid}/download") Object history(@PathVariable String id,@PathVariable String rid,@RequestParam(defaultValue="pdf") String format,HttpServletRequest r) { var c=contract(id,r); require(Set.of("docx","pdf").contains(format),422,"INVALID_FORMAT","文件格式无效"); var rev=revision(rid); require(id.equals(rev.get("contract_id"))&&rid.equals(str(c,"finalized_revision_id")),404,"NOT_FOUND","历史定稿不存在"); var j=db.find("SELECT * FROM generation_jobs WHERE revision_id=? AND state='SUCCEEDED'",rid); require(j!=null,409,"PREVIEW_NOT_READY","该历史修订的生成产物不存在，请联系管理员"); return templates.download(str(j,format.equals("pdf")?"pdf_file_id":"docx_file_id"),str(c,"contract_no")+"-FINAL-R"+rev.get("revision_no")+"."+format,false,r,"HISTORY_DOWNLOADED",rid); }
